...
def getflux(ischeme, islope, dt, dx, A, Sigma, L, R, u):
    ...
    if ischeme == "godunov_sys_m":
        fh = godunov_sys_m(islope, dx, A, Sigma, L, R, u)
    elif ischeme == "lax_friedrichs_sys_m":
        fh = lax_friedrichs_sys_m(islope, dx, dt, A, u)
    elif ischeme == "rusanov_sys_m":
        fh = rusanov_sys_m(islope, A, u)
    else:
...
def lax_friedrichs_sys_m(islope, dx, dt, A, u):
    ...
    fh = 0.5 * A @ (um + up) - 0.5 * dx/dt * (up - um)
    return fh

def rusanov_sys_m(islope, A, u):
    Sigma_ab = np.absolute(Sigma)
    ...
    up = cprojectbk(R, wp)
    lambda_max = 2
    fh = 0.5 * A @ (um + up) - 0.5 * lambda_max * (up - um)
    return fh
...
if __name__ == "__main__":
    ...
    R = np.array([[-2, 2], [1, 1]])
    L = np.array([[-1/4, 1/2], [1/4, 1/2]]) # R^-1
    ...
    icase = 3
    # icase 1: "godunov_sys",
    # icase 3: "godunov_sys_m", "lax_friedrichs_sys_m", "rusanov_sys_m"
    ischeme = "rusanov_sys_m"
    islope = "minmod"
    # "minmod", "superbee", "mc"
    ...
    for im in range(m):
        with open ('num_sol_{}_{}_u{}.txt'.format(ischeme, islope, im+1), 'w', encoding='utf-8') as file:
            for index in range(len(xi)):
                file.write("%f %f %f \n"%(xi[index], uex[im, index], uc_[im, index]))

    plt.figure()
    for im in range(m):
        plt.plot(xi, uc_[im, :], 'rD', markerfacecolor="None", markersize=5, label= f'{islope}, {ischeme}')
