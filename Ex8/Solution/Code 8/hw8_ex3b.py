...
"u_0(x) = sin(4*pi*x), x=[0, 1]"
    
def init(dx, x):
    u0_ = np.zeros(len(x)-1)
    for i in range(len(x)-1):
        u0_[i] = 1/(dx*4*np.pi)*(np.cos(4*np.pi*x[i])-np.cos(4*np.pi*x[i+1]))
    return u0_
    
def exactu(t, x):
    uex = lambda t, x: np.sin(4*np.pi*(x-t))
    u = uex(t, x)
    return u
...
if __name__ == "__main__":
    ...
    # icase 3: "lax-friedrichs_m", "rusanov_m", "godunov_m"
    ischeme = "godunov_m"
    islope = "vanleer"
    ...
    with open ('num_sol_{}_{}_sm.txt'.format(ischeme, islope), 'w', encoding='utf-8') as file:
